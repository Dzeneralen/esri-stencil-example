...
@Component({
  ...
  export class AppMap {
    ...
    constructor() {
      ...
      ], this.esriMapOptions).then(([EsriMap, FeatureLayer]: [__esri.MapConstructor, __esri.FeatureLayerConstructor]) => {
    ...
    createEsriMapView() {
      return loadModules([
        'esri/views/MapView'
      ], this.esriMapOptions).then(([EsriMapView] : [__esri.MapViewConstructor]) => {
    ...
    zoomToUrlObjectId(duration = 1600) {
      if (this.match && this.match.params && this.match.params.objectid) {
        this.municipalitiesFeatureLayer.queryFeatures({
    ...
    addZoomOnClickAndUrlUpdate() {
      this.esriMapView.on("click", (evt) => {
        ...
        this.esriMapView.whenLayerView(this.municipalitiesFeatureLayer).then((lyrView: __esri.FeatureLayerView) => {
